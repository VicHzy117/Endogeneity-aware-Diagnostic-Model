...
make_binary_design <- function(K) {
  ...
  # vapply simplifies to a vector when K=1. Restore the intended K-by-2^K
  # matrix so that the candidate dimension K=1 is fitted correctly.
  if (K == 1L) bits <- matrix(bits, nrow = 1L)
  cbind(1, t(bits))
}
...
ECDM_main <- function(Y, V, covarites, K_a, K_g, iteration,
  ...
  pi2_list <- vector("list", iteration + 1L)
  Q1_list[[1]] <- Q_MH_1
  Q2_list[[1]] <- Q_MH_2
  B_list[[1]] <- B_i
  L_list[[1]] <- L_i
